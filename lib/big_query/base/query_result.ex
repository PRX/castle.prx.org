defmodule BigQuery.Base.QueryResult do
  ...
  def parse_meta(data) do
    %{
      ...
      megabytes:
        data |> Map.get("totalBytesProcessed", "0") |> String.to_integer() |> div(1000 * 1000),
      changed: data |> Map.get("numDmlAffectedRows", "0") |> String.to_integer()
    }
  end
  ...
  defp parse_value(value, type) do
    case {type, value} do
      ...
      {"RECORD", %{"v" => v}} ->
        parse_value(v, "RECORD")

      {"RECORD", %{"f" => f}} ->
        parse_value(f, "RECORD")

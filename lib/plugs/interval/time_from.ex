defmodule Castle.Plugs.Interval.TimeFrom do
  ...
  def parse(conn), do: parse(conn, %{})
  def parse(%{params: %{"from" => from_dtim}}, _opts) when is_bitstring(from_dtim) do
  ...
  def parse(_conn, _opts), do: {:error, "Missing required param: from"}

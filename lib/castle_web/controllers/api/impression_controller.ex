defmodule CastleWeb.API.ImpressionController do
  ...
  defp cache_podcast(intv, id) do
    @redis.interval "impressions.podcasts", intv, id, fn(new_intv) ->
      @bigquery.podcast_impressions(new_intv)
    end
  end

  defp cache_episode(intv, guid) do
    @redis.interval "impressions.episodes", intv, guid, fn(new_intv) ->
  ...
  defp group_cache_podcast(intv, group, id) do
    ...
    @redis.cached key("podcast.#{id}", intv_nobucket, group), @group_ttl, fn() ->
      @bigquery.podcast_impressions(id, intv_nobucket, group)
    end
  end
  ...
  defp group_cache_episode(intv, group, guid) do
    ...
    @redis.cached key("episode.#{guid}", intv_nobucket, group), @group_ttl, fn() ->
      @bigquery.episode_impressions(guid, intv_nobucket, group)
    end
  end

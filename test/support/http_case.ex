defmodule Castle.HttpCase do
  ...
  def mock_full_url(url, _hdrs, nil), do: url

  def mock_full_url(url, _hdrs, opts) do
    case opts[:params] do
      nil -> url
  ...
  defmacro test_with_http(name, mocks, do: expression) do
    quote do
      test unquote(name) do
        getter = fn url, hdrs, opts ->
          full_url = mock_full_url(url, hdrs, opts)
        ...
        getter_no_opts = fn url, hdrs ->
          full_url = mock_full_url(url, hdrs, %{})
        ...
        with_mock HTTPoison, get: getter, get: getter_no_opts do
          unquote(expression)
        end
      end
    end

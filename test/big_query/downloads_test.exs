defmodule Castle.BigQueryDownloadsTest do
  ...
  test "lists downloads for all podcasts" do
    intv = interval("2017-06-27T21:00:00Z", "2017-06-28T04:00:00Z", BigQuery.TimestampRollups.Hourly)
    ...
    assert Map.get(counts, 57) == 7289
  end

  test "groups downloads for a podcast" do
    intv = interval("2017-07-10T21:00:00Z", "2017-07-11T04:00:00Z", BigQuery.TimestampRollups.Hourly)
    group = Castle.Plugs.Group.get("city", 3)
    {result, _meta} = group_podcast(57, intv, group)

    assert is_list result
    assert length(result) == 7 * 4

    assert_time result, 0, "2017-07-10T21:00:00Z"
    assert_time result, 1, "2017-07-10T21:00:00Z"
    assert_time result, 2, "2017-07-10T21:00:00Z"
    assert_time result, 3, "2017-07-10T21:00:00Z"
    assert_time result, 4, "2017-07-10T22:00:00Z"
    assert_time result, 7 * 4 - 1, "2017-07-11T03:00:00Z"
  ...
  test "lists downloads for all episodes" do
    ...
    assert_time time1, "2017-06-27T21:00:00Z"
    assert_time time2, "2017-06-27T22:00:00Z"
    assert length(Map.keys(counts)) > 10
    assert Map.has_key?(counts, "e4f5a88b-b383-493a-b7f3-8b8ea52cbf35")
    assert Map.get(counts, "e4f5a88b-b383-493a-b7f3-8b8ea52cbf35") == 1004
  end

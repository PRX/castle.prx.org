defmodule Castle.API.TotalViewTest do
  ...
  test "total.json" do
    ...
    assert Enum.at(doc.downloads, 0) == %{code: "AB", label: "fakelabel", count: 10}
    assert Enum.at(doc.downloads, 1) == %{code: "CD", label: "fakelabel", count: 9}
    assert Enum.at(doc.downloads, 2) == %{code: "EF", label: "fakelabel", count: 8}
    assert Enum.at(doc.downloads, 3) == %{code: "GH", label: "fakelabel", count: 7}
  end
end
